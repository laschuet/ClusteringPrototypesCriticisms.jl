...
"""
function output(D, protoids, critids, title; color=nothing)
    println("$title:")
    ...
    fig = Figure()
    Axis(fig[1, 1], xgridvisible=false, ygridvisible=false, limits=(0, 6, 0, 4), xlabel=L"x", ylabel=L"y", title=title)
    if isnothing(color)
        scatter!(D[1, :], D[2, :], markersize=4)
    else
        scatter!(D[1, :], D[2, :], color=color, markersize=4)
    end
    scatter!(protos[1, :], protos[2, :], color=:red, marker=:cross)
    scatter!(crits[1, :], crits[2, :], color=:green, marker=:xcross)
    save("out/$(replace(title, ' ' => '_')).pdf", fig)
end
...
function main()
    ...
    output(D, protoids, critids, "k-medoids", color=assignments(clustering))

    # k-means
    clustering = kmeans(D, k)
    protoids = prototypes(clustering, p)
    critids = criticisms(clustering, c)
    output(D, protoids, critids, "k-means", color=assignments(clustering))

    # fuzzy c-means
    clustering = fuzzy_cmeans(D, k, 2)
    protoids = prototypes(clustering, p)
    critids = criticisms(clustering, c)
    output(D, protoids, critids, "fuzzy c-means", color=map(i -> i[2], argmax(clustering.weights, dims=2)))
    ...
    output(D, protoids, critids, "affinity propagation", color=assignments(clustering))

    # MMD-critic
    kernel = with_lengthscale(RBFKernel(), sqrt(size(D, 1)))
    protoids = prototypes(D, kernel, k)
    critids = criticisms(D, kernel, protoids, k)
    output(D, protoids, critids, "MMD-critic", color=:grey)
end
